/**
 * AS3935_test.ino — Standalone test sketch for AS3935 sensor driver
 *
 * ISS Packet Engine — Sensor Driver Tests
 *
 * Wiring (Teensy 4.x):
 *   AS3935 SDA → Pin 18 (SDA)
 *   AS3935 SCL → Pin 19 (SCL)
 *   AS3935 IRQ → Pin 7  (IRQ 07)
 *   AS3935 VCC → 3.3V
 *   AS3935 GND → GND
 *   AS3935 SDO → open (address 0x03)
 * Input: Interrupt signal (high) from AS3935 to indicate activity 
...
void setup()
{
  ...
  pinMode(lightningInt, INPUT); 
    ...
    Serial.println();
    Serial.println("========================================");
    Serial.println("  AS3935 Lightning Sensor Driver Test");
    Serial.println("========================================");
    Serial.println();

  Wire.begin(); // Begin Wire before lightning sensor. 
      // --- Initialization ---
    Serial.print("Initializing AS3935 at 0x");
    Serial.print(AS3935_ADDR, HEX);
    Serial.print("... ");
...
void loop()
{
  if(digitalRead(lightningInt) == HIGH){
    ...
    if(intVal == NOISE_INT){
      ...
      Serial.print(distance * 0.621371); 
      Serial.println("mi away!"); 
